import { IconWithBorder, IconWithBorderProps } from './IconWithBorder';

type IconWithBranches = IconWithBorderProps & {
  subText?: string;
  branches: string[];
};

export function IconWithBranches({
  subText,
  branches,
  ...iconProps
}: IconWithBranches) {
  return (
    <div className="flex">
      <div className="ml-2 z-10">
        <IconWithBorder {...iconProps} />
        {subText && (
          <div className="text-[10px] opacity-50 w-16 mt-1 absolute">
            {subText}
          </div>
        )}
      </div>
